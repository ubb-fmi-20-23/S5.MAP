...
class AssistantActivity : Activity(), Button.OnButtonEventListener {
  ...
  private val mAssistantResponseObserver = object : StreamObserver<AssistResponse> {
    override fun onNext(value: AssistResponse) {
      ...
      if (value.deviceAction != null && !value.deviceAction.deviceRequestJson.isEmpty()) {
        // Iterate through JSON object
        try {
          val deviceAction = JSONObject(value.deviceAction.deviceRequestJson)
          val inputs = deviceAction.getJSONArray("inputs")
          for (i in 0 until inputs.length()) {
            if (inputs.getJSONObject(i).getString("intent") == "action.devices.EXECUTE") {
              ...
              for (j in 0 until commands.length()) {
                ...
                for (k in 0 until execution.length()) {
                  val command = execution.getJSONObject(k)
                      .getString("command")
                  val params = execution.getJSONObject(k)
  ...
  fun handleDeviceAction(command: String, params: JSONObject) {
    when (command) {
      "action.devices.commands.OnOff" -> {
        val paramValue = params.getBoolean("on")
        if (paramValue) {
          display.displayMessage("ON")
        } else {
          display.displayMessage("")
        }
        logi("Display: $paramValue")
      }
      "com.example.commands.Display" -> {
        val number = params.getInt("number")
        display.displayMessage("$number")
        logi("Display: $number")
      }
      else -> logi("Received an unknown command: $command")
    }
  }
